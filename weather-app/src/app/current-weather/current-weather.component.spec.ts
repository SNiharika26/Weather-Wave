import { ComponentFixture, TestBed } from '@angular/core/testing';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';
import { Weather } from '../weather.model';
import { WeatherService } from '../weather.service';

import { CurrentWeatherComponent } from './current-weather.component';
import { Router } from '@angular/router';

describe('CurrentWeatherComponent', () => {
  let component: CurrentWeatherComponent;
  let fixture: ComponentFixture<CurrentWeatherComponent>;
  let weatherServiceSpy: jasmine.SpyObj<WeatherService>;
  let routerSpy: jasmine.SpyObj<Router>;

  beforeEach(async () => {
    const spy = jasmine.createSpyObj('WeatherService', ['getWeather']);
    const routerSpyObj = jasmine.createSpyObj('Router', ['navigate']); // Create a spy for the Router service

    await TestBed.configureTestingModule({
      declarations: [CurrentWeatherComponent],
      imports: [RouterTestingModule.withRoutes([])], // Import RouterTestingModule here
      providers: [
        { provide: WeatherService, useValue: spy },
        { provide: Router, useValue: routerSpyObj } // Provide the spy Router service
      ]
    })
    .compileComponents();
    
    fixture = TestBed.createComponent(CurrentWeatherComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
    weatherServiceSpy = TestBed.inject(WeatherService) as jasmine.SpyObj<WeatherService>;
    routerSpy = TestBed.inject(Router) as jasmine.SpyObj<Router>;
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });

  it('should fetch current weather data', () => {
    // Set up the mock data
    const mockWeather: Weather = {
      days: [],
      description: 'The characteristics of someone or something',
      id: 1,
      latitude: 10.0,
      longitude: 10.0,
      queryCost: 1,
      timezone: 'UTC',
      address: ''
    };
    
    // Set up the spy to return the mock data
    weatherServiceSpy.getWeather.and.returnValue(of(mockWeather));
  
    // Set the city in the component
    component.city = 'London';
  
    // Call the method to fetch current weather
    component.getCurrentWeather();
  
    // Check if the weather data in the component matches the mock data
    expect(component.weatherData).toEqual(mockWeather);
  });
  
  it('should save favorite location to local storage', () => {
    spyOn(localStorage, 'setItem');

    component.city = 'Paris';
    component.saveFavoriteLocation();

    expect(localStorage.setItem).toHaveBeenCalledWith('favoriteLocation', 'Paris');
    expect(window.alert).toHaveBeenCalledWith('Saved Paris as favorite location!');
  });

  it('should navigate to home page', () => {
    component.navigateToHome();

    expect(routerSpy.navigate).toHaveBeenCalledWith(['weather']); // Use routerSpy.navigate instead of component.router.navigate
  });

  it('should toggle showAllData property', () => {
    component.showAllData = false;
    component.toggleShowAllData();
    expect(component.showAllData).toBe(true);

    component.toggleShowAllData();
    expect(component.showAllData).toBe(false);
  });

});
